...
#if 0 //TODO
	connect(account->myself(), SIGNAL(onlineStatusChanged(Contact *,
			const OnlineStatus &, const OnlineStatus &)),
		this, SLOT(slotAccountOnlineStatusChanged(Contact *,
			const OnlineStatus &, const OnlineStatus &)));
#endif
	connect(account, SIGNAL(accountDestroyed(const Account *)) , this, SLOT( unregisterAccount(const Account *) ));
...
{
	connect( PluginManager::self(), SIGNAL( pluginLoaded( Plugin * ) ),
	...
	for ( QStringList::Iterator it = accountGroups.begin(); it != accountGroups.end(); ++it )
	{
		...
		if ( protocol.endsWith( QString::fromLatin1( "Protocol" ) ) )
			protocol = QString::fromLatin1( "_" ) + protocol.lower().remove( QString::fromLatin1( "protocol" ) );
