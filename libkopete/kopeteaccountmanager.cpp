    ...
    foreach (Account *account, d->accounts) {
        ...
        if (onlyChangeConnectedAccounts) {   //If global status is offline, change all account to new status
            if (account->isConnected()
                || (((flags & ConnectIfOffline) || Kopete::StatusManager::self()->globalStatusCategory() == Kopete::OnlineStatusManager::Offline) && !account->excludeConnect())) {
                account->setOnlineStatus(status, statusMessage);
            }
        } else {
...
void AccountManager::slotAccountOnlineStatusChanged(Contact *c, const OnlineStatus &oldStatus, const OnlineStatus &newStatus)
{
