...
private:
    ...
    bool registerTemplate(const QString &key, const PropertyTmpl &tmpl);
    void unregisterTemplate(const QString &key);

    const PropertyTmpl &createProp(const QString &key, const QString &label, const QString &icon = QString(), bool persistent = false) const;
