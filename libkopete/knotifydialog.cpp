...
void Application::save()
{
    ...
    Event *e;
    while ( (e = it.current()) ) {
		config->setGroup( "Event/" + e->configGroup );
		int n = KNotification::staticMetaObject.indexOfEnumerator( "NotifyPresentation" );
		config->writeEntry( "Action", QString(KNotification::staticMetaObject.enumerator(n).valueToKeys(e->presentation) ));
        config->writePathEntry( "sound", e->soundfile );
...
void Application::reloadEvents( bool revertToDefaults )
{
    ...
    while ( it != conflist.end() ) {
        if ( rx.indexIn(*it) > -1 ) { // event group
            kc->setGroup( *it );
            ...
            if ( e->name.isEmpty() || e->description.isEmpty() )
				...
				const QString& default_rep = kc->readEntry("Action");
                QString default_logfile = kc->readPathEntry("default_logfile");
                QString default_soundfile = kc->readPathEntry("sound");
