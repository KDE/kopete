...
KStaticDeleter<PluginManager> pluginmanager_sd;
...
{
	if ( !s_self )
		pluginmanager_sd.setObject( s_self, new PluginManager() );

	return s_self;
}

PluginManager::PluginManager() : QObject( qApp )
{
	d = new Private;
...
QPtrList<Plugin> PluginManager::loadedPlugins( const QString &category ) const
{
	QPtrList<Plugin> result;
	QMap<KPluginInfo *, Plugin *>::ConstIterator it;
...
void PluginManager::slotShutdownTimeout()
{
	...
	for ( QMap<KPluginInfo *, Plugin *>::ConstIterator it = d->loadedPlugins.begin(); it != d->loadedPlugins.end(); ++it )
