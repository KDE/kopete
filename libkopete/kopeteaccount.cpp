...
Account::Account(Protocol *parent, const QString &accountId)
    : QObject(parent)
    , d(new Private(parent, accountId))
{
...
void Account::disconnected(DisconnectReason reason)
{
    ...
    if (reason == OtherClient) {
        Kopete::Utils::notifyConnectionLost(this, i18n("You have been disconnected"), i18n("You have connected from another client or computer to the account '%1'", d->id),
                                            i18n(
    ...
    if (!protocol()->canAddMyself() && contactId == d->myself->contactId()) {
        if (isConnected() && d->lastLoginTime.secsTo(QDateTime::currentDateTime()) > 30) {
            KMessageBox::queuedMessageBox(Kopete::UI::Global::mainWidget(), KMessageBox::Error,
                                          i18n("You are not allowed to add yourself to the contact list. The addition of \"%1\" to account \"%2\" will not take place.", contactId, accountId()),
                                          i18n("Error Creating Contact")
                                          );
        } else {
...
void Account::slotOnlineStatusChanged(Contact * /* contact */, const OnlineStatus &newStatus, const OnlineStatus &oldStatus)
{
...
void Account::slotContactPropertyChanged(PropertyContainer * /* contact */, const QString &key, const QVariant &old, const QVariant &newVal)
{
