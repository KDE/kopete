...
Kopete::FileTransferInfo::FileTransferInfo(Kopete::Contact *contact, const QStringList &files, const unsigned long size, const QString &recipient, KopeteTransferDirection di, const unsigned int id,
                                           QString internalId, const QPixmap &preview, bool saveToDirectory)
{
    mContact = contact;
    mFiles = files;
    mId = id;
    ...
    , d(new Private(kfti))
{
    ...
    QUrl targ;
    targ.setPath(d->info.file());
    init(displayURL(d->info.contact(), targ.fileName()), showProgressInfo);
}

void Kopete::Transfer::init(const QUrl &target, bool showProgressInfo)
{
    d->target = target;
...
QUrl Kopete::Transfer::sourceURL()
{
    if (d->info.direction() == Kopete::FileTransferInfo::Incoming) {
        return displayURL(d->info.contact(), d->info.file());
    } else {
        QUrl url;
        url.setPath(d->info.file());
        return url;
    }
}

QUrl Kopete::Transfer::destinationURL()
{
    return d->target;
}

void Kopete::Transfer::emitCopying(const QUrl &src, const QUrl &dest)
{
    emit description(this, i18n("Copying"),
                     qMakePair(i18n("Source"), src.toDisplayString()),
                     qMakePair(i18n("Destination"), dest.toDisplayString()));
...
Kopete::Transfer *Kopete::TransferManager::addTransfer(Kopete::Contact *contact, const QString &file, const unsigned long size, const QString &recipient,
                                                       Kopete::FileTransferInfo::KopeteTransferDirection di)
{
    return addTransfer(contact, QStringList(file), size, recipient, di);
}

Kopete::Transfer *Kopete::TransferManager::addTransfer(Kopete::Contact *contact, const QStringList &files, const unsigned long size, const QString &recipient,
                                                       Kopete::FileTransferInfo::KopeteTransferDirection di)
{
...
unsigned int Kopete::TransferManager::askIncomingTransfer(Kopete::Contact *contact, const QString &file, const unsigned long size, const QString &description, QString internalId,
                                                          const QPixmap &preview)
{
    return askIncomingTransfer(contact, QStringList(file), size, description, internalId, preview);
}

unsigned int Kopete::TransferManager::askIncomingTransfer(Kopete::Contact *contact, const QStringList &files, const unsigned long size, const QString &description, QString internalId,
                                                          const QPixmap &preview)
{
...
void Kopete::TransferManager::saveIncomingTransfer(unsigned int id)
{
    ...
    KConfigGroup cg(KSharedConfig::openConfig(), "File Transfer");
    const QString defaultPath = cg.readEntry("defaultPath", QDir::homePath());
    QUrl url = QString(defaultPath + QLatin1String("/") + info.file());
    ...
    const QString directory = (info.saveToDirectory()) ? url.toLocalFile() : url.adjusted(QUrl::RemoveFilename|QUrl::StripTrailingSlash).path();
...
void Kopete::TransferManager::sendFile(const QUrl &file, const QString &fname, unsigned long sz, bool mustBeLocal, QObject *sendTo, const char *slot)
{
    QUrl url = file;
    QString filename;
    unsigned int size = 0;
    ...
    if (!url.isEmpty()) {
        if (mustBeLocal && !url.isLocalFile()) {
            ...
            connect(this, SIGNAL(sendFile(QUrl,QString,uint)), sendTo, slot);
            emit sendFile(url, filename, size);
            disconnect(this, SIGNAL(sendFile(QUrl,QString,uint)), sendTo, slot);
        }
    }
}
...
QUrl Kopete::TransferManager::getSaveFile(const QUrl &startDir) const
{
    ...
    for (;;) {
        ...
        if (fileInfo.exists()) {
            ...
            QFileInfo dirInfo(url.adjusted(QUrl::RemoveFilename|QUrl::StripTrailingSlash).path());
...
QUrl Kopete::TransferManager::getSaveDir(const QUrl &startDir) const
{
    QUrl url = startDir;
    for (;;) {
