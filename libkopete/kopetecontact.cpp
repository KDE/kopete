...
Contact::Contact(Account *account, const QString &contactId, MetaContact *parent, const QString &icon)
...
//TODO PORT QT5     wVBoxLayout->setSpacing( QDialog::spacingHint() );
    ...
    if (moveDialog->exec() == QDialog::Accepted) {
        ...
        if (chkCreateNew->isChecked()) {
            ...
            if (metaContact()) { // Add new metaContact to old groups so we don't move it to Top Level group
...
void Contact::serialize(QMap<QString, QString> & /*serializedData*/, QMap<QString, QString> & /* addressBookData */)
{
}
...
QString Kopete::Contact::formattedIdleTime() const
{
    ...
    if (leftTime > 0) { // FIXME: duplicated from code in kopetecontact listview.cpp
...
void Kopete::Contact::slotPropertyChanged(Kopete::PropertyContainer *, const QString &key, const QVariant &, const QVariant &)
{
