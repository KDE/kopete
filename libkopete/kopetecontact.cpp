	...
	: QObject( parent )
{
	...
	{
	#if 0
	 	connect( parent, SIGNAL( aboutToSave( MetaContact * ) ),
			protocol(), SLOT( slotMetaContactAboutToSave( MetaContact * ) ) );
	#endif
...
KPopupMenu* Contact::popupMenu( MessageManager *manager )
{
	...
	d->actionUserInfo    = KopeteStdAction::contactInfo( this, SLOT( userInfo() ),          menu, "actionUserInfo" );
...
void Contact::slotChangeMetaContact()
{
	...
	QPtrList<MetaContact> metaContacts = ContactList::contactList()->metaContacts();
...
void Contact::setMetaContact( MetaContact *m )
{
	...
	if( old )
	{
		...
		disconnect( old, SIGNAL( aboutToSave( MetaContact * ) ),
			protocol(), SLOT( slotMetaContactAboutToSave( MetaContact * ) ) );
#if 0 //TODO
		old->updateKABC();
#endif
...
#if 0 //TODO
		connect( d->metaContact, SIGNAL( aboutToSave( MetaContact * ) ),
			protocol(), SLOT( slotMetaContactAboutToSave( MetaContact * ) ) );
		m->updateKABC();
#endif
	}
