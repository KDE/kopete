...
{ public:
	...
	Message::MessageImportance importance;
	bool bgOverride;
	bool fgOverride;
	bool rtfOverride;
	QDateTime timeStamp;
...
Message::Message() : d(new Private)
{
	init( QDateTime::currentDateTime(), 0L, QPtrList<Contact>(), QString::null, QString::null, Internal, PlainText, Chat );
}

Message::Message( const Contact *fromKC, const QPtrList<Contact> &toKC, const QString &body,
	MessageDirection direction, MessageFormat f, MessageType type )   : d(new Private)
{
...
Message::Message( const Contact *fromKC, const QPtrList<Contact> &toKC, const QString &body,
	const QString &subject, MessageDirection direction, MessageFormat f, MessageType type )  : d(new Private)
{
...
Message::Message( const QDateTime &timeStamp, const Contact *fromKC, const QPtrList<Contact> &toKC,
	const QString &body, MessageDirection direction, MessageFormat f, MessageType type )  : d(new Private)
{
	init( timeStamp, fromKC, toKC, body, QString::null, direction, f, type );
}

Message::Message( const QDateTime &timeStamp, const Contact *fromKC, const QPtrList<Contact> &toKC,
	const QString &body, const QString &subject, MessageDirection direction, MessageFormat f, MessageType type )  : d(new Private)
{
...
void Message::init( const QDateTime &timeStamp, const Contact *from, const QPtrList<Contact> &to,
...
QString Message::parsedBody() const
{
	...
	{
#if 0
		return Emoticons::parseEmoticons(parseLinks(escapedBody(), d->format));
#endif
	}
}
...
MessageManager *Message::manager() const
{
	return d->manager;
}

void Message::setManager(MessageManager *kmm)
{
	detach();
	d->manager=kmm;
}
...
const QDomDocument Message::asXML() const
{
	...
	///for( Contact *c = d->to.first(); c; c = d->to.next() )
	//{
		Contact *c = d->to.first();
		if( c )
		{
