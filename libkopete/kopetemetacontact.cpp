...
: ContactListElement( ContactList::self() )
{
	...
	connect( this, SIGNAL( removedFromGroup( MetaContact *, Group * ) ), SIGNAL( persistentDataChanged() ) );
	connect( this, SIGNAL( addedToGroup( MetaContact *, Group * ) ), SIGNAL( persistentDataChanged() ) );
...
void MetaContact::addContact( Contact *c )
{
	...
	else
	{
		...
		connect( c, SIGNAL( onlineStatusChanged( Contact *, const OnlineStatus &, const OnlineStatus & ) ),
			SLOT( slotContactStatusChanged( Contact *, const OnlineStatus &, const OnlineStatus & ) ) );

		connect( c, SIGNAL( propertyChanged( Contact *, const QString &, const QVariant &, const QVariant & ) ),
			this, SLOT( slotPropertyChanged( Contact *, const QString &, const QVariant &, const QVariant & ) ) ) ;

		connect( c, SIGNAL( contactDestroyed( Contact * ) ),
			this, SLOT( slotContactDestroyed( Contact * ) ) );

		connect( c, SIGNAL( idleStateChanged( Contact * ) ),
			this, SIGNAL( contactIdleStateChanged( Contact * ) ) );
...
{
#if 0 //TODO
	OnlineStatus::OnlineStatus newStatus = OnlineStatus::Unknown;
	OnlineStatus mostSignificantStatus;
	...
	newStatus = OnlineStatus::OnlineStatus( mostSignificantStatus.status() );
...
void MetaContact::removeContact(Contact *c, bool deleted)
{
	if( !d->contacts.contains( c ) )
	{
		kdDebug(14010) << "MetaContact::removeContact: Contact is not in this metaContact " << endl;
	}
	else
	{
		d->contacts.remove( c );
		...
		{  //If this function is tell by slotContactRemoved, c is maybe just a QObject
			disconnect( c, SIGNAL( onlineStatusChanged( Contact *, const OnlineStatus &, const OnlineStatus & ) ),
				this, SLOT( slotContactStatusChanged( Contact *, const OnlineStatus &, const OnlineStatus & ) ) );

			connect( c, SIGNAL( propertyChanged( Contact *, const QString &, const QVariant &, const QVariant & ) ),
			this, SLOT( slotPropertyChanged( Contact *, const QString &, const QVariant &, const QVariant & ) ) ) ;


			disconnect( c, SIGNAL( contactDestroyed( Contact * ) ),
				this, SLOT( slotContactDestroyed( Contact * ) ) );

			disconnect( c, SIGNAL( idleStateChanged( Contact * ) ),
				this, SIGNAL( contactIdleStateChanged( Contact *) ) );
...
{
#if 0 //TODO
	switch ( Prefs::prefs()->interfacePreference() )
	{
...
QString MetaContact::statusIcon() const
{
	switch( status() )
	{
		...
		default:
			if( useCustomIcon() )
				return icon( PluginDataObject::Offline );
			else
...
OnlineStatus::OnlineStatus MetaContact::status() const
{
