...
Contact *MetaContact::preferredContact()
{
    ...
    */
    ...
    while (it.hasNext())
    {
        ...
        if (c->manager(Contact::CannotCreate)) { //no need to check the view. having a manager is enough
            if (!hasOpenView) {
                contact = c;
                hasOpenView = true;
                if (c->isReachable()) {
        ...
        if (!contact) { //this is the first contact.
            contact = c;
            continue;
        }
...
QString MetaContact::displayName() const
{
    ...
    if (source == SourceKABC) {
        ...
        if (d->displayNameSourceContact == 0) {
            if (d->contacts.count() >= 1) {// don't call setDisplayNameSource , or there will probably be an infinite loop
...
void MetaContact::slotPropertyChanged(PropertyContainer *_subcontact, const QString &key, const QVariant &oldValue, const QVariant &newValue)
{
...
void MetaContact::onlineStatusNotification(Kopete::Contact *c)
{
    ...
    if (!c->account()->suppressStatusNotification() && c->account()->isConnected()
        ...
        enum ChangeType {
            noChange, noEvent, signedIn, changedStatus, signedOut
        };
