...
*/
bool UPnp::sendAction(QUrl &deviceSettingUrl, Service &service, QString nameAction, QList<QString> paramNameAction,QList<QString> paramValueAction)
{
	bool find = false;
	int ret;
	...
	if(this->isValidUrl(deviceSettingUrl) && !service.isEmpty())
	{
		if(service.existAction(nameAction)==true)
		{
			...
			{
				actionNode = UpnpMakeAction( nameAction.toLatin1().data(), service.serviceType().toLatin1().data(), 0,NULL );
			}
			else
			{
				for( int k=0; k < paramNameAction.size(); k++ ) 
				{
					...
					if( UpnpAddToAction( &actionNode, nameAction.toLatin1().data(), service.serviceType().toLatin1().data(),nameActionP.toLatin1().data(),valueActionP.toLatin1().data()) != UPNP_E_SUCCESS ) 
					{
			...
			char * relURL = (char *)malloc( strlen( deviceSettingUrl.toString().toLatin1().data() ) + strlen( service.controlURL().toLatin1().data() ) + 1 );
			
			ret = UpnpResolveURL( deviceSettingUrl.toString().toLatin1().data(), service.controlURL().toLatin1().data(), relURL );
			...
			ret = UpnpSendActionAsync( 
				...
				service.serviceType().toLatin1().data(),
				NULL, 
				actionNode,
