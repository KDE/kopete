...
$EXTRACTRC `find . -name \*.ui -o -name \*.rc | egrep -v '(libkopete/compat|protocols/testbed)'` >> rc.cpp || exit 12
LIST=`find . -name \*.h -o -name \*.cpp -o -name \*.c | egrep -v '(libkopete/compat|protocols/testbed|protocols/jabber/kioslave)'`
if test -n "$LIST"; then
