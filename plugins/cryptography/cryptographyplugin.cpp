...
void CryptographyPlugin::slotIncomingMessage(Kopete::MessageEvent *messageEvent)
{
    ...
    connect(decryptVerifyJob, SIGNAL(result(GpgME::DecryptionResult,GpgME::VerificationResult,QByteArray)), this,
            SLOT(slotIncomingMessageContinued(GpgME::DecryptionResult,GpgME::VerificationResult,QByteArray)));
        ...
        if (protocol != "Kopete::Protocol") {
            if (!supportedProtocols().contains(protocol)) {
                KMessageBox::information(0,
                                         i18nc("@info",
                                               "This protocol %1 may not work with messages that are encrypted. This is because encrypted messages are very long, and the server or peer may reject them due to their length. To avoid being signed off or your account being warned or temporarily suspended, turn off encryption.",
                                               protocol),
...
QString CryptographyPlugin::kabcKeySelector(QString displayName, QString addresseeName, QStringList keys, QWidget *parent)
{
    ...
    if (keys.count() == 1) {
        if (KMessageBox::questionYesNo(parent,
                                       i18nc("@info", "Cryptography plugin has found an encryption key for %1 (%2) in your KDE address book. Do you want to use key %3 as this contact's public key? ",
                                             displayName, addresseeName,
                                             keys.first().right(8).prepend("0x")),
