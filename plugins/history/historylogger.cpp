...
{
    if (m_realMonth != QDate::currentDate().month()) { //We changed month, our index is not correct anymore, clean memory.
    ...
    if (!m_metaContact) { //this may happen if the contact has been moved, and the MC deleted
        if (c && c->metaContact()) {
            m_metaContact = c->metaContact();
        } else {
            return QDomDocument();
...
void HistoryLogger::appendMessage(const Kopete::Message &msg, const Kopete::Contact *ct)
{
    ...
    if (!m_toSaveFileName.isEmpty() && m_toSaveFileName != filename) { //that mean the contact or the month has changed, save it now.
        saveToDisk();
    }
    ...
    foreach (Kopete::Contact *contact, ct) {
        ...
        while (!n.isNull())
        {
            ...
            if (!msgElem2.isNull() && msgElem2.tagName() == "msg") {
                ...
                if (!m_hideOutgoing || dir != Kopete::Message::Outbound) { //parse only if we don't hide it
...
QList<Kopete::Message> HistoryLogger::readMessages(int lines, const Kopete::Contact *c, Sens sens, bool reverseOrder, bool colorize)
{
    ...
    if (m_oldSens != Default && sens != m_oldSens) { //we changed our sens! so retrieve the old position to fly in the other way
    ...
    while (messages.count() < lines)
    {
        ...
        if (!c && m_metaContact->contacts().count() > 1) { //we have to merge the differents subcontact history
            QList<Kopete::Contact *> ct = m_metaContact->contacts();

            foreach (Kopete::Contact *contact, ct) { //we loop over each contact. we are searching the contact with the next message with the smallest date,
