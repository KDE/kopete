...
void OtrlChatInterface::handle_msg_event(void *opdata, OtrlMessageEvent msg_event, ConnContext *context, const char *message, gcry_error_t err)
{
    ...
    case OTRL_MSGEVENT_CONNECTION_ENDED:
        msg.setHtmlBody(i18n("%1 has already closed his/her private connection to you. Your message was not sent. Either end your private conversation, or restart it.",
                             QLatin1String(context->username)));
        msg.setDirection(Kopete::Message::Internal);
        break;
...
int OtrlChatInterface::decryptMessage(Kopete::Message &message)
{
    ...
    int ignoremessage = otrl_message_receiving(userstate, &ui_ops, chatSession, accountId.toLocal8Bit(), protocol.toLocal8Bit(), contactId.toLocal8Bit(),
                                               body.toLocal8Bit(), &newMessage, &tlvs, &context, NULL, NULL);
...
int OtrlChatInterface::encryptMessage(Kopete::Message &message)
{
    ...
    if (otrl_proto_message_type(msgBody.toLocal8Bit()) == OTRL_MSGTYPE_NOTOTR) {
        ...
        int err = otrl_message_sending(userstate, &ui_ops, chatSession, accountId.toLocal8Bit(), protocol.toLocal8Bit(), contactId.toLocal8Bit(), instance,
                                       msgBody.toUtf8(), NULL, &newMessage, OTRL_FRAGMENT_SEND_ALL_BUT_LAST, NULL, NULL, NULL);
...
void OtrlChatInterface::disconnectSession(Kopete::ChatSession *chatSession)
{
    ...
    otrl_message_disconnect(userstate, &ui_ops, chatSession, chatSession->account()->accountId().toLatin1(), chatSession->account()->protocol()->displayName().toLatin1(),
...
int OtrlChatInterface::privState(Kopete::ChatSession *session)
{
    ...
    ConnContext *context = otrl_context_find(userstate, session->members().first()->contactId().toLocal8Bit(), session->account()->accountId().toLocal8Bit(),
                                             session->account()->protocol()->displayName().toLocal8Bit(), instance, 0, NULL, NULL, NULL);
...
void OtrlChatInterface::verifyFingerprint(Kopete::ChatSession *session)
{
    ...
    otrl_instag_t instance = session->property("otr-instag").toUInt();
    context = otrl_context_find(userstate, session->members().first()->contactId().toLocal8Bit(), session->account()->accountId().toLocal8Bit(),
