...
void AuthenticationWizard::updateInfoBox(){
	if(rbQA->isChecked()){
		...
		infoLabel->setText(i18n("Pick a secret known only to you and %1. If the secres don't match, you may be talking to an imposer. Do not send the secret through the chat window, or this authentication method could be compromised with ease.", OtrlChatInterface::self()->formatContact(session->members().first()->contactId())));
	} else {
