            ...
            if (question != NULL || rbQA->isChecked()) {
                if (initiate) {
                    ...
                    if (trust) {
                        ...
                        lFinal->setText(i18n("<b>%1</b> has successfully authenticated you. You may want to authenticate this contact as well by asking your own question.",
                                             OtrlChatInterface::self()->formatContact(session->members().first()->contactId())));
                    }
                }
            } else {
...
void AuthenticationWizard::aborted()
{
    ...
    lFinal->setText(i18n("%1 has aborted the authentication process. To make sure you are not talking to an imposter, try again using the manual fingerprint verification method.",
                         OtrlChatInterface::self()->formatContact(session->members().first()->contactId())));
...
{
    if (rbQA->isChecked()) {
        infoLabel->setText(i18n("Ask %1 a question, the answer to which is known only to you and them. If the answer does not match, you may be talking to an imposter.",
                                OtrlChatInterface::self()->formatContact(session->members().first()->contactId())));
    } else if (rbSS->isChecked()) {
        infoLabel->setText(i18n(
        ...
        infoLabel->setText(i18n("Verify %1's fingerprint manually. For example via a phone call or signed (and verified) email.",
                                OtrlChatInterface::self()->formatContact(session->members().first()->contactId())));
    }
}
