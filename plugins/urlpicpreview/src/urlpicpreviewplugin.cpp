 ...
 */
QString URLPicPreviewPlugin::prepareBody(const QString& parsedBody) {
        ...
        if(KIO::NetAccess::mimetype(url, Kopete::UI::Global::mainWidget()).startsWith("image/")) {
            if(KIO::NetAccess::download(url, tmpFile, Kopete::UI::Global::mainWidget())) {
                ...
                if(m_pic.load(tmpFile)) {
                    ...
                    kdDebug(0) << k_funcinfo << "Couldn't load image " << tmpFile << endl;
                }
                ...
                return myParsedBody + prepareBody(ex.cap(6));
            }
        } else {
            kdDebug(0) << k_funcinfo << foundURL << " is not an image file. Ignoring." << endl;
        }
    } else {
        kdDebug(0) << k_funcinfo << "URL \"" << foundURL << "\" is invalid. Ignoring." << endl;
    }

    return myParsedBody.replace(QRegExp(rex), ex.cap(1) + ex.cap(2) + ex.cap(3) + ex.cap(4) + ex.cap(5)) + prepareBody(ex.cap(6));
}
