...
void NLKscd::update()
{
	...
	if (  m_client->isApplicationRegistered( "kscd" ) )
	{
					...
					replyType, replyData ) )
		{
			...
			m_playing = true;
			kdDebug() << "NLKscd::update() - KsCD without playing()"
				<< endl;
		}
		else
		{
		...
		if ( !m_client->call( "kscd", "CDPlayer",
			...
			kdDebug( ) <<  "NLKscd::update() DCOP error" 
				<< endl;
		else {
			QDataStream reply( replyData, IO_ReadOnly );
			if ( replyType == "QStringList" ) {
				...
				kdDebug() << "NLKscd::update() trackList returned unexpected reply type!"
					<< endl;
			// Get the current track title
			if ( !m_client->call( "kscd", "CDPlayer",
						"currentTrackTitle()", data, replyType, replyData ) )
				kdDebug( ) << "NLKscd::update() - there was some error using DCOP." << endl;
			else {
				QDataStream reply( replyData, IO_ReadOnly );
				if ( replyType == "QString" ) {
					reply >> newTrack;
					//kdDebug() << "the result is: " << newTrack.latin1()
					//	<< endl;
				} else
					kdDebug() << "NLKscd::update()-  currentTrackTitle "
		...
		if ( newTrack != m_track )
		{
			...
			m_newTrack = false;
		kdDebug() << "NLKscd::update() - found kscd - " 
			<< m_track << endl;
