...
bool History2Logger::messageExists(const Kopete::Message &msg, const Kopete::Contact *ct)
{
    ...
    query.prepare(
        "SELECT 1 FROM history WHERE direction = :direction AND protocol = :protocol AND account= :account AND me_id = :me_id AND other_id = :other_id AND datetime = :datetime AND message = :message");
...
QList<Kopete::Message> History2Logger::readMessages(int lines, int offset, const Kopete::MetaContact *c, bool reverseOrder)
{
    QList<Kopete::Message> messages;
    Kopete::Account *account;
    Kopete::Contact *from;
