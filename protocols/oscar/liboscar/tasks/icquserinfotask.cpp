...
bool ICQUserInfoRequestTask::take( Transfer* transfer )
{
	if ( forMe( transfer ) )
	{
		...
		case 0x00FA:  //affliations user info
			kDebug( OSCAR_RAW_DEBUG ) << k_funcinfo << "Received organization & affliation info";
			...
			break;
		case 0x0104:
			kDebug( OSCAR_RAW_DEBUG ) << k_funcinfo << "Received short user info";
			shortInfo.setSequenceNumber( seq );
			shortInfo.fill( buffer );
			m_shortInfoMap[seq] = shortInfo;
			break;
		case 0x010E:  //homepage category user info
			kDebug( OSCAR_RAW_DEBUG ) << k_funcinfo << "Got homepage category info, but we don't support it yet";
			break;
		default:
			break;
		}
...
void ICQUserInfoRequestTask::onGo()
{
	...
	if ( m_type != Short )
	{
		...
		kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "Requesting full user info for " << m_userToRequestFor;
	}
	else
	{
		setRequestSubType( 0x04BA );
		kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "Requesting short user info for " << m_userToRequestFor;
	}
