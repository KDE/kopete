...
void TypingNotifyTask::handleNotification()
{
	...
	{
	case 0x0000:
		kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << contact << " has finished typing";
		emit typingFinished( contact );
		break;
	case 0x0001:
		kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << contact << " has typed a word";
		emit typingFinished( contact );
		break;
	case 0x0002:
		kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << contact << " has started typing";
		emit typingStarted( contact );
		break;
	default:
		kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << contact << " typed an unknown typing notification - " << word;
	}
}
