...
void SSIListTask::handleContactListReply()
{
	...
	kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "SSI Protocol version: " << buffer->getByte();
	Oscar::WORD ssiItems = buffer->getWord();
	kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "Number of items in this SSI packet: " << ssiItems;
	...
	for ( parsedItems = 1; parsedItems <= ssiItems; ++parsedItems )
	{
		...
		kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "Got SSI Item: " << s.toString();
	...
	if ( buffer->bytesAvailable() > 0 )
	{
		...
		{
			kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "SSI List complete";
			client()->ssiManager()->setListComplete( true );
			setSuccess( 0, QString() );
		}
		else
			kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "Awaiting another SSI packet";
	}
...
{
	kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "Our SSI List is up to date";
	Buffer* buffer = transfer()->buffer();

	client()->ssiManager()->setLastModificationTime( buffer->getDWord() );
	Oscar::WORD ssiItems = buffer->getWord();
	kDebug(OSCAR_RAW_DEBUG) << k_funcinfo << "Number of items in SSI list: " << ssiItems;
...
{
	kDebug( OSCAR_RAW_DEBUG ) << k_funcinfo << "Checking the timestamp of the SSI list";
	FLAP f = { 0x02, 0, 0 };
