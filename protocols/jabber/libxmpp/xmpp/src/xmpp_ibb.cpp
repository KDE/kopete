...
bool IBBConnection::isConnected() const
{
	if(d->state == Active)
		return true;
	else
...
bool IBBConnection::canRead() const
{
	return (!d->recvbuf.isEmpty());
}
...
void IBBConnection::trySend()
{
	...
	if(doClose)
		...
		printf("(%d bytes left)", d->sendbuf.size());
