...
{
	kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "New incoming transfer from " << incomingTransfer->peer().full () << ", filename " << incomingTransfer->fileName () << ", size " << QString::number ( incomingTransfer->fileSize () );
	...
	if ( !contact )
	{
		kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "No matching local contact found, creating a new one.";
...
{
	kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "New outgoing transfer for " << contact->contactId() << ": " << file;
...
{
	kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "Destroying Jabber file transfer object.";
...
void JabberFileTransfer::slotIncomingTransferAccepted ( Kopete::Transfer *transfer, const QString &fileName )
{
	...
	kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "Accepting transfer for " << mXMPPTransfer->peer().full ();
...
void JabberFileTransfer::slotTransferRefused ( const Kopete::FileTransferInfo &transfer )
{
	...
	kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "Local user refused transfer from " << mXMPPTransfer->peer().full ();
...
void JabberFileTransfer::slotTransferResult ()
{
	...
	{
		kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "Transfer with " << mXMPPTransfer->peer().full () << " has been canceled.";
		mXMPPTransfer->close ();
		deleteLater ();
	}
...
void JabberFileTransfer::slotIncomingDataReady ( const QByteArray &data )
{
	...
	{
		kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "Transfer from " << mXMPPTransfer->peer().full () << " done.";
...
void JabberFileTransfer::slotOutgoingConnected ()
{
	...
	kDebug ( JABBER_DEBUG_GLOBAL ) << k_funcinfo << "Outgoing data connection is open.";
...
void JabberFileTransfer::slotOutgoingBytesWritten ( int nrWritten )
{
	...
	{
		kDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "Transfer to " << mXMPPTransfer->peer().full () << " done.";
