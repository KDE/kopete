...
void JabberProtocol::connect() {
	...
	QObject::connect(jabberClient,
			 ...
			 this,
			 SLOT(slotGroupChatPresence
			      (const Jid &, const Status &)));

	QObject::connect(jabberClient,
			 SIGNAL(groupChatError
				(const Jid &, int, const QString &)), this,
    ...
    else if (proxyTypeStr == QString("SOCKS4"))
    ...
    Jabber::StreamProxy proxy(proxyType,
			      ...
			      KGlobal::config()->readNumEntry("ProxyPort",
							      8080));
    ...
    kdDebug(JABBER_DEBUG_GLOBAL) <<
	"[JabberProtocol] Connecting to Jabber server " << server << ":" << port
	<< endl;
    kdDebug(JABBER_DEBUG_GLOBAL) << "                 with UserID " << userId <<
	endl;
...
void JabberProtocol::slotHandshaken() {
    ...
    kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] Performing login..." <<
	endl;
	...
	if (KGlobal::config()->readEntry("AuthType", "digest")
	    ...
	    jabberClient->authDigest(KGlobal::config()->readEntry("UserID", ""),
				     KGlobal::config()->readEntry("Password",
								  ""),
				     KGlobal::config()->readEntry("Resource",
								  "Kopete"));
	}
	else {
...
void JabberProtocol::slotConnected(bool success, int statusCode,
    ...
    if (success) {
	kdDebug(JABBER_DEBUG_GLOBAL) <<
	    "[JabberProtocol] Connected to Jabber server." << endl;
	...
	kdDebug(JABBER_DEBUG_GLOBAL) <<
	    "[JabberProtocol] Connection failed! Status: " << statusCode << ", "
	    << statusString << endl;
...
void JabberProtocol::disconnect() {
    kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] disconnect() called" <<
	endl;

    if (isConnected()) {
	kdDebug(JABBER_DEBUG_GLOBAL) <<
	    "[JabberProtocol] Still connected, closing connection..." << endl;
...
void JabberProtocol::slotDisconnected() {
    kdDebug(JABBER_DEBUG_GLOBAL) <<
	"[JabberProtocol] Disconnected from Jabber server." << endl;
...
void JabberProtocol::slotError(const Jabber::StreamError & error) {
	...
	KMessageBox::error(qApp->mainWidget(),
			   i18n("Connection refused (%1)").arg(error.details(),
							       1),
			   i18n("Error Connecting to Jabber Server"));
	break;
	...
	KMessageBox::error(qApp->mainWidget(),
			   i18n("Remote server closed connection (%1)").
			   arg(error.details(), 1),
			   i18n("Error Connecting to Jabber Server"));
	break;
	...
	KMessageBox::error(qApp->mainWidget(),
			   i18n("Handshake failed (%1)").arg(error.details(),
							     1),
			   i18n("Error Connecting to Jabber Server"));
	break;
    ...
    default:
	KMessageBox::error(qApp->mainWidget(),
			   i18n("An unknown error was encountered (%1)").
			   arg(error.details(), 1),
			   i18n("Error Connecting to Jabber Server"));
	break;
    }
 ...
 */
void JabberProtocol::setPresence(const KopeteOnlineStatus & status,
	...
	kdDebug(JABBER_DEBUG_GLOBAL) << k_funcinfo << "Updating to \"" <<
	    presence.status()
...
void JabberProtocol::deserializeContact(KopeteMetaContact * metaContact,
    ...
    new JabberContact(serializedData["contactId"],
		      serializedData["displayName"], QStringList::split(',',
									serializedData
									["groups"]),
...
void JabberProtocol::slotGoChatty() {
    kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] Setting 'chatty' mode." <<
	endl;
...
void JabberProtocol::slotGoAway() {
    kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] Setting away mode." <<
	endl;
...
void JabberProtocol::slotGoXA() {
    kdDebug(JABBER_DEBUG_GLOBAL) <<
	"[JabberProtocol] Setting extended away mode." << endl;
...
void JabberProtocol::slotGoDND() {
    kdDebug(JABBER_DEBUG_GLOBAL) <<
	"[JabberProtocol] Setting do not disturb mode." << endl;
...
void JabberProtocol::slotGoInvisible() {
    kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] Setting invisible mode."
	<< endl;
...
void JabberProtocol::sendPresenceToNode(const KopeteOnlineStatus & pres,
...
void JabberProtocol::createAddContact(KopeteMetaContact * mc,
	...
	if (mc) {
		...
		kdDebug(JABBER_DEBUG_GLOBAL) <<
		    "[JabberProtocol]****Warning**** : " << item.jid().
		    userHost()
...
void JabberProtocol::slotSubscription(const Jabber::Jid & jid,
	...
	kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] slotSubscription(): "
	    << jid.userHost()
	    << " asks for authorization to subscribe." << endl;
	...
	KMessageBox::information(0L,
				 i18n("%1 unsubscribed you!").arg(jid.
								  userHost()),
				 i18n("Notification"));
...
void JabberProtocol::slotContactUpdated(const Jabber::RosterItem & item) {
    ...
    if (!contacts()[item.jid().userHost()]) {
	kdDebug(JABBER_DEBUG_GLOBAL) <<
	    "[JabberProtocol] WARNING: slotContactUpdated() " <<
...
void JabberProtocol::slotReceivedMessage(const Jabber::Message & message) {
	...
	kdDebug(JABBER_DEBUG_GLOBAL) <<
	    "[JabberProtocol] New server message for us!" << endl;
	...
	if (!contactFrom) {
	    ...
	    kdDebug(JABBER_DEBUG_GLOBAL) <<
		"[JabberProtocol] Message received from an " <<
...
void JabberProtocol::slotGroupChatJoined(const Jabber::Jid & jid) {
    kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] Joined group chat " <<
	jid.full() << endl;
...
void JabberProtocol::slotGroupChatLeft(const Jabber::Jid & jid) {
    kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] Left groupchat " << jid.
	full() << endl;
...
void JabberProtocol::slotGroupChatPresence(const Jabber::Jid & jid,
    ...
    kdDebug(JABBER_DEBUG_GLOBAL) <<
	"[JabberProtocol] Received groupchat presence for room " << jid.
	full() << endl;

    static_cast<JabberGroupChat *>(contacts()[jid.userHost()])->updatePresence(jid,
									status);
...
void JabberProtocol::slotGroupChatError(const Jabber::Jid & /* jid */ ,
					int /* error */ ,
					QString & /* reason */ ) {
    kdDebug(JABBER_DEBUG_GLOBAL) << "[JabberProtocol] Group chat error!" <<
	endl;
...
void JabberProtocol::slotResourceAvailable(const Jabber::Jid & jid,
    ...
    kdDebug(JABBER_DEBUG_GLOBAL) <<
	"[JabberProtocol] New resource available for " << jid.
	userHost() << endl;

    if (!contacts()[jid.userHost()]) {
	kdDebug(JABBER_DEBUG_GLOBAL) <<
	    "[JabberProtocol] Trying to add a resource, but " <<
...
void JabberProtocol::slotResourceUnavailable(const Jabber::Jid & jid,
    ...
    kdDebug(JABBER_DEBUG_GLOBAL) <<
	"[JabberProtocol] Resource now unavailable for " << jid.
	userHost() << endl;

    if (!contacts()[jid.userHost()]) {
	kdDebug(JABBER_DEBUG_GLOBAL) <<
	    "[JabberProtocol] Trying to remove a resource, " <<
