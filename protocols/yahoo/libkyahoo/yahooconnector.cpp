...
KNetworkConnector::KNetworkConnector( QObject *parent )
		: Connector( parent )
{
	kDebug( YAHOO_RAW_DEBUG ) << k_funcinfo << "New KNetwork connector.";
...
{
	Q_UNUSED( server );
	kDebug( YAHOO_RAW_DEBUG ) << k_funcinfo << "Initiating connection to " << mHost;
...
{
	kDebug( YAHOO_RAW_DEBUG ) << k_funcinfo << "We are connected.";
...
{
	kDebug( YAHOO_RAW_DEBUG ) << k_funcinfo << "Error detected: " << code;
...
{
	kDebug ( YAHOO_RAW_DEBUG ) << k_funcinfo << "Manually specifying host " << host << " and port " << port;
