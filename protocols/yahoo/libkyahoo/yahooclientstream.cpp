...
void ClientStream::cp_outgoingData( const QByteArray& outgoingBytes )
{
	...
	kDebug(YAHOO_RAW_DEBUG) << k_funcinfo << "[data size: " << outgoingBytes.size() << "]";
...
// 	kDebug(YAHOO_RAW_DEBUG) << k_funcinfo;
	...
	if ( incoming )
	{
// 		kDebug(YAHOO_RAW_DEBUG) << k_funcinfo << " - got a new transfer";
		d->in.enqueue( incoming );
		d->newTransfers = true;
		emit doReadyRead();
	}
	else
		kDebug(YAHOO_RAW_DEBUG) << k_funcinfo << " - client signalled incomingData but none was available, state is: "<< d->client.state();
}
...
// 	kDebug(YAHOO_RAW_DEBUG) << k_funcinfo;
	...
	//kDebug(YAHOO_RAW_DEBUG) << k_funcinfo << " recv: " << a.size()  <<" bytes";
	//cs_dump( a );
...
{
	kDebug(YAHOO_RAW_DEBUG) << k_funcinfo << " written: " << bytes  <<" bytes";
}
