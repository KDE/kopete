...
{}
void MessengerContact::sync(unsigned int)
{}
void MessengerContact::deleteContact()
{}
...
QList<KAction *> *MessengerContact::customContextMenuActions()
{
	...
	QString blockLabel = i18n( "Unblock User" );// = isBlocked() ? i18n( "Unblock User" ) : i18n( "Block User" );
	if( !actionBlock )
	{
	...
	//actionSendMail->setEnabled( static_cast<MessengerAccount*>(account())->isLivemail());
...
void MessengerContact::sendFile( const KUrl &sourceURL, const QString &altFileName, uint /*fileSize*/ )
{
	...
	//kDebug(14140) << "MessengerContact::sendFile: File chosen to send:" << fileName << endl;
