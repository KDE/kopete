/*
    ligcontact.cpp - Kopete Lig Protocol
...
LigContact::LigContact( Kopete::Account* _account, const QString &uniqueName,
		const LigContactType type, const QString &displayName, Kopete::MetaContact *parent )
: Kopete::Contact( _account, uniqueName, parent )
{
	...
	setOnlineStatus( LigProtocol::protocol()->ligOffline );
}

LigContact::~LigContact()
{
}

bool LigContact::isReachable()
{
    return true;
}

void LigContact::serialize( QMap< QString, QString > &serializedData, QMap< QString, QString > & /* addressBookData */ )
{
    QString value;
	switch ( m_type )
	{
...
Kopete::ChatSession* LigContact::manager( CanCreateFlags )
{
...
QPtrList<KAction> *LigContact::customContextMenuActions() //OBSOLETE
{
...
void LigContact::showContactSettings()
{
	//LigContactSettings* p = new LigContactSettings( this );
	//p->show();
}

void LigContact::sendMessage( Kopete::Message &message )
{
	kdDebug( 14210 ) << k_funcinfo << endl;
	// convert to the what the server wants
	// For this 'protocol', there's nothing to do
	// send it
	static_cast<LigAccount *>( account() )->server()->sendMessage(
...
void LigContact::receivedMessage( const QString &message )
{
...
void LigContact::slotChatSessionDestroyed()
{
	//FIXME: the chat window was closed?  Take appropriate steps.
	m_msgManager = 0L;
}

#include "ligcontact.moc"
