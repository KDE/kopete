...
LigWebcamDialog::LigWebcamDialog( const QString &contactId, QWidget * parent, const char * name )
...
LigWebcamDialog::~ LigWebcamDialog( )
{
	mVideoDevicePool->stopCapturing();
	mVideoDevicePool->close();
}

void LigWebcamDialog::slotUpdateImage()
{
	mVideoDevicePool->getFrame();
	mVideoDevicePool->getImage(&mImage);
