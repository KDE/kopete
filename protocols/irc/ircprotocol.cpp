...
void IRCProtocol::editNetworks( const QString &networkName )
{
	if( !netConf )
	{
		...
		connect( netConf->networkList, SIGNAL( doubleClicked ( Q3ListBoxItem * )), SLOT(slotRenameNetwork()));		
	}
...
void IRCProtocol::slotMoveServerDown()
{
	...
	{
		Q3ValueList<IRCHost*>::iterator nextPos = selectedNetwork->hosts.remove( pos );
