...
bool ChatPropertiesTask::take( Transfer * transfer )
{
				...
				if ( mf->tag() == NM_A_FA_CHAT_ACL )
				{
										 ...
										 ++aclIt )
					{
						...
						if ( aclEntryFields )
						{
							...
							//kDebug ( GROUPWISE_DEBUG_GLOBAL ) << "got acl entry: " << entry.dn << ", " << entry.chatRights;
	...
	//kDebug ( GROUPWISE_DEBUG_GLOBAL ) << "Got chatroom properties: " << m_chat << " : " << m_ownerDn << ", " << m_description << ", " << m_disclaimer << ", " << m_query << ", " << m_archive << ", " << m_topic << ", " << m_creatorDn << ", " << m_creationTime.toString() << ", " << m_rights;
	finished();
	return true;
}
