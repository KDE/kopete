...
void Webcam::processMessage(const Message& message)
{
	...
	if(message.applicationIdentifier != 4l)
	{
		...
		kDebug(14141) << k_funcinfo << "received, " << body;
	...
	kDebug(14141) << k_funcinfo << dataMessage.size() << echoS;
#endif
	...
	else if(m_content.startsWith("ack"))
	{
		...
		if(m_who==wProducer)
		{
			...
			kDebug(14140) << k_funcinfo << "m_myAuth= " << m_myAuth;
			QString  producerxml=xml(sess , rid);
			kDebug(14140) << k_funcinfo << "producerxml= " << producerxml; 
	...
	else if(m_content.contains("<producer>")   ||  m_content.contains("<viewer>"))
	{
		...
		kDebug(14140) << k_funcinfo << (isListening ? QString("listening %1").arg(m_listener->localAddress().toString()) : QString("not listening"));
		...
		while(true)
		{
			...
			if(!port1.isEmpty())
			{
				kDebug(14140) << k_funcinfo << "trying to connect on " << ip <<':' << port1;
				...
				kDebug(14140) << k_funcinfo << "okok " << sock << " - " << sock->peerAddress().toString() << " ; " << sock->localAddress().toString();
			}
			if(!port2.isEmpty())
			{
				kDebug(14140) << k_funcinfo << "trying to connect on " << ip <<':' << port2;
			...
			if(!port3.isEmpty())
			{
				kDebug(14140) << k_funcinfo << "trying to connect on " << ip <<':' << port3;
		...
		for ( it = m_allSockets.begin(); it != m_allSockets.end(); ++it )
		{
			...
			kDebug(14140) << k_funcinfo << "connect to "  << sock << " - "<< sock->peerAddress().toString() << " ; " << sock->localAddress().toString();
		}
	}
...
int Webcam::getAvailablePort()
{
    ...
    kDebug(14140) << k_funcinfo<< "found available port : " << port;
...
void Webcam::slotSocketConnected()
{
	...
	kDebug(14140) << k_funcinfo << "Connection established on  " <<  m_webcamSocket->peerAddress().toString() << " ; " << m_webcamSocket->localAddress().toString();
...
//	QObject::connect(m_webcamSocket, SIGNAL(gotError(int)), this, SLOT(slotSocketError(int)));
	...
	kDebug(14140) << k_funcinfo << "sending "<< m_peerAuth;
...
void Webcam::slotSocketRead()
{
	...
	kDebug(14140) << k_funcinfo  <<  m_webcamSocket <<  "############# " << available << " bytes available.";
	...
	switch(m_webcamState)
	{
		case wsNegotiating:
		{
			...
			{
				kDebug(14140) << k_funcinfo << "waiting more data   ( " << available << "  of  " <<m_myAuth.length()<< " )";
				break;
			}
			QByteArray buffer(available);
			m_webcamSocket->read(buffer.data(), buffer.size());
		
			kDebug(14140) << k_funcinfo << buffer.data();

			if(QString(buffer) == m_myAuth )
			{
				closeAllOtherSockets();
				kDebug(14140) << k_funcinfo << "Sending " << connected_str;
				...
				if(m_who==wProducer)
				{
					...
					kDebug(14140) << k_funcinfo <<  "new timer" << m_timerId;
				}
			...
			{
				kWarning(14140) << k_funcinfo << "Auth failed";
		...
		case wsConnected:
		{
			...
			{
				kDebug(14140) << k_funcinfo << "waiting more data   ( " << available << "  of  " <<connected_str.length()<< " )";
				break;
			}
			QByteArray buffer(connected_str.length());
			m_webcamSocket->read(buffer.data(), buffer.size());
			
// 			kDebug(14140) << k_funcinfo << "state " << m_webcamState << " received :" << QCString(buffer);
			...
			{
				kWarning(14140) << k_funcinfo << "Connecting failed";
				m_webcamSocket->disconnect();
				m_webcamSocket->deleteLater();
				m_allSockets.remove(m_webcamSocket);
				m_webcamSocket=0l;
		...
		case wsTransfer:
		{
			if(m_who==wProducer)
			{
				kWarning(14140) << k_funcinfo << "data received when we are producer";
				break;
			}
			if(available < 24)
			{
				kDebug(14140) << k_funcinfo << "waiting more data   ( " << available << "  of  " <<24<< " )";
				break;
			}
			QByteArray buffer(available);
			...
			{
				kDebug(14140) << k_funcinfo << "waiting more data   ( " << available << "  of  " <<paysize<< " )";
				break;
			}
			...
			if(pix.isNull())
			{
				kWarning(14140) << k_funcinfo << "incorrect pixmap returned, better to stop everything";
				m_webcamSocket->disconnect();
				sendBYEMessage();
			}
...
{
	kWarning(14140) << k_funcinfo << "Error " << errorCode << " : " << m_listener->errorString();
}
...
//	kDebug(14140) << k_funcinfo;
	...
	{
		kWarning(14140) << k_funcinfo << "Bad image size " <<img.width() << 'x' <<  img.height();
		return;
	}
	...
	kDebug(14140) << k_funcinfo << "Sendinf frame of size " << frame.size();
	//build the header.
	QByteArray header;
