...
void Webcam::processMessage(const Message& message)
{
	...
	else if(m_content.startsWith("ack"))
	{
		...
		if(m_who==wProducer)
		{
			...
			kDebug(14140) << "m_myAuth= " << m_myAuth;
			QString  producerxml=xml(sess , rid);
			kDebug(14140) << "producerxml= " << producerxml; 
	...
	else if(m_content.contains("<producer>")   ||  m_content.contains("<viewer>"))
	{
		...
		kDebug(14140) << (isListening ? QString("listening %1").arg(m_listener->localAddress().toString()) : QString("not listening"));
		...
		while(true)
		{
			...
			if(!port1.isEmpty())
			{
				kDebug(14140) << "trying to connect on " << ip <<':' << port1;
				...
				kDebug(14140) << "okok " << sock << " - " << sock->peerAddress().toString() << " ; " << sock->localAddress().toString();
			}
			if(!port2.isEmpty())
			{
				kDebug(14140) << "trying to connect on " << ip <<':' << port2;
			...
			if(!port3.isEmpty())
			{
				kDebug(14140) << "trying to connect on " << ip <<':' << port3;
		...
		for ( it = m_allSockets.begin(); it != m_allSockets.end(); ++it )
		{
			...
			kDebug(14140) << "connect to "  << sock << " - "<< sock->peerAddress().toString() << " ; " << sock->localAddress().toString();
		}
	}
...
int Webcam::getAvailablePort()
{
    ...
    kDebug(14140) << "found available port : " << port;
...
void Webcam::slotSocketConnected()
{
	...
	kDebug(14140) << "Connection established on  " <<  m_webcamSocket->peerAddress().toString() << " ; " << m_webcamSocket->localAddress().toString();
...
//	QObject::connect(m_webcamSocket, SIGNAL(gotError(int)), this, SLOT(slotSocketError(int)));
	...
	kDebug(14140) << "sending "<< m_peerAuth;
...
void Webcam::slotSocketRead()
{
	...
	kDebug(14140) <<  m_webcamSocket <<  "############# " << available << " bytes available.";
	...
	switch(m_webcamState)
	{
		case wsNegotiating:
		{
			...
			{
				kDebug(14140) << "waiting more data   ( " << available << "  of  " <<m_myAuth.length()<< " )";
				break;
			}
			QByteArray buffer(available);
			m_webcamSocket->read(buffer.data(), buffer.size());
		
			kDebug(14140) << buffer.data();

			if(QString(buffer) == m_myAuth )
			{
				closeAllOtherSockets();
				kDebug(14140) << "Sending " << connected_str;
				...
				if(m_who==wProducer)
				{
					...
					kDebug(14140) <<  "new timer" << m_timerId;
				}
		...
		case wsConnected:
		{
			...
			{
				kDebug(14140) << "waiting more data   ( " << available << "  of  " <<connected_str.length()<< " )";
				break;
			}
			QByteArray buffer(connected_str.length());
			m_webcamSocket->read(buffer.data(), buffer.size());
			
// 			kDebug(14140) << "state " << m_webcamState << " received :" << QCString(buffer);
			...
			{
				kWarning(14140) << "Connecting failed";
				m_webcamSocket->disconnect();
				m_webcamSocket->deleteLater();
				m_allSockets.remove(m_webcamSocket);
				m_webcamSocket=0l;
		...
		case wsTransfer:
		{
			if(m_who==wProducer)
			{
				kWarning(14140) << "data received when we are producer";
				break;
			}
			if(available < 24)
			{
				kDebug(14140) << "waiting more data   ( " << available << "  of  " <<24<< " )";
				break;
			}
			QByteArray buffer(available);
			...
			{
				kDebug(14140) << "waiting more data   ( " << available << "  of  " <<paysize<< " )";
				break;
			}
			...
			if(pix.isNull())
			{
				kWarning(14140) << "incorrect pixmap returned, better to stop everything";
				m_webcamSocket->disconnect();
				sendBYEMessage();
			}
...
{
	kWarning(14140) << "Error " << errorCode << " : " << m_listener->errorString();
}
...
//	kDebug(14140) ;
	...
	{
		kWarning(14140) << "Bad image size " <<img.width() << 'x' <<  img.height();
		return;
	}
	...
	kDebug(14140) << "Sendinf frame of size " << frame.size();
	//build the header.
	QByteArray header;
