...
QPixmap MimicWrapper::decode(const QByteArray& data)
{
	if(!m_init)
	{
		...
		{
			kWarning(14140) << k_funcinfo << "Impossible to init decoder";
			return QPixmap();
		}
		if (!mimic_get_property( m_mimctx, "buffer_size", &m_bufferSize) )
		{
			kWarning(14140) << k_funcinfo << "Impossible to get buffer size";
			return QPixmap();
		}
		m_init=true;
	}
	...
	{
		kWarning(14140) << k_funcinfo << "Impossible to decode frame";
		return QPixmap();
	}
	int width,height;
...
QByteArray MimicWrapper::encode(const QByteArray& data)
{
	if(!m_init)
	{
		...
		{
			kWarning(14140) << k_funcinfo << "Impossible to init encoder";
			return QByteArray();
		}
		if (!mimic_get_property( m_mimctx, "buffer_size", &m_bufferSize) )
		{
