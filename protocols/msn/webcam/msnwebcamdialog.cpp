	...
	m_imageContainer( this )
{
	...
	*/
	...
	if ( page )
	{
		kDebug(14180) << k_funcinfo << "Adding webcam image container";
...
{
	kDebug(14180) << k_funcinfo << "New image received";
	//	kDebug(14180) << image;
...
{
	kDebug(14180) << k_funcinfo << "webcam closed with reason?? " <<  reason;
// 	m_imageContainer.clear();
