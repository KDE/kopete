...
void ChatView::updateChatState(KopeteTabState newState)
{
    if (newState == Undefined) {
        newState = m_tabState;
               ...
               && (newState != Message || m_tabState != Highlighted)) { //if the new state is not a typing state and we don't already have a message or a highlighted message
...
void ChatView::saveOptions()
{
    ...
    KConfigGroup kopeteChatWindowMainWinSettings(config, (msgManager()->form() == Kopete::ChatSession::Chatroom ? QStringLiteral("KopeteChatWindowGroupMode") : QStringLiteral(
...
void ChatView::dropEvent(QDropEvent *event)
{
    ...
    if (event->mimeData()->hasFormat(QStringLiteral("application/kopete.metacontacts.list"))) {
        ...
        for (QList<QUrl>::Iterator it = urlList.begin(); it != urlList.end(); ++it) {
            if ((*it).isLocalFile()) { //send a file
                contact->sendFile(*it);
            } else { //this is a URL, send the URL in a message
                addText((*it).url());
            }
        }
        event->accept();
