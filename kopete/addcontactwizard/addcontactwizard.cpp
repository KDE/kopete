...
// FIXME: we SHOULDN'T use names of variables that are generated by UIC!, this one has changed, and there is trouble.
// please fix, once this whole piece compiles and (somehow) works
//	selectAddressee->addWidget(m_addressbookSelectorWidget);
	...
	QListIterator<Kopete::Account *> accit(accounts);
	while ( accit.hasNext() )
	{
...
void AddContactWizard::accept()
{
	...
	for ( it = protocolPages.begin(); it != protocolPages.end(); ++it )
		ok = ok || it.value()->apply( it.key(), metaContact );
		...
		(currentPage() == intro && !appropriate( selectService )))
	{
		...
		for (Q3ListViewItemIterator it(protocolListView); it.current(); ++it)
		{
			...
			if (item && item->isOn())
			{
				...
				insertPage( addPage, i18nc( "The user has to select the contact to add to the given account name", 
					"Choose New Contact For %1 Account <b>%2</b>", i->protocol()->displayName(), item->text(0) ), indexOf( finis ) );
