...
bool KopeteContactListView::acceptDrag(QDropEvent *e) const
{
	...
	if( const_cast<const QWidget *>( e->source() ) == this )
	{
		...
		//check to make sure the MCLVI is valid. Also check to make sure it's
		//not coming from the awaiting auth group. If so, deny drop
		if(source_metaLVI)
		{
			source_contact = source_metaLVI->contactForPoint( m_startDragPos );
			if ( source_metaLVI->group()->type() == KopeteGroup::AwaitingAuth )
				return false;
		}
